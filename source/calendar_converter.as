/*
 This is a public-domain library of functions from fourmilab.com (John Walker), found
 online at http://www.fourmilab.com/documents/calendar/

 It provides conversions between these calendars: Julian, Gregorian, Islamic (civil),
 Hebrew, Persian, Mayan, Bahai, French Revolutionary, Indian (civil). It also handles
 UTC and ISO time formats.

 NOTE that this requires access to the functions in the script "astro.js", which adds
 pretty thorough astronomical (sidereal?) logic, useful for reckoning in those calendrical
 systems that depend on phenomena like sunsets, equinoces, etc.

 Note that this file was originally an HTML page at the URL above. I'm extracting just
 its functions for use in Flash, and migrating the UI and form widgets into Flash. Any
 of my modifications will be commented and marked with 'jimA'.

 Jim Allman, Interrobang Digital Media (jim@ibang.com)
 April 2003
*/
...
function calcJulian()
{
	//jimA: Changing local variable 'date' to 'gdate', to make sure we don't
	// obscure the built-in Date object
    var j, gdate, time;

    j = new Number(document.julianday.day.value);
    gdate = jd_to_gregorian(j);
    time = jhms(j);
    document.gregorian.year.value = gdate[0];
    document.gregorian.month.selectedIndex = gdate[1] - 1;
    document.gregorian.day.value = gdate[2];
